...
char * managewww(int sock,int fd){
  ...
  *ret='\0';
  quit=0;
  rr=recvfrom(sock,buf,100,0,(struct sockaddr *)&from,&fromlen);
  if(rr<1)return ret;
  for(i=0;i<totaccess;i++)if(memcpm(&(from.sin6_addr),access+i,sizeof(struct in6_addr))break;
  if(i==totaccess)return ret;
  *(buf+rr)='\0';
